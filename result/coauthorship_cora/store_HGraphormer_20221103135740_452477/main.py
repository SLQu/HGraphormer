...
if __name__ == '__main__':
    ...
    for run in range(1, args.nruns+1):
        ...
        for epoch in range(args.epochs):
            ...
            sub_H = torch.tensor(prepare.getNumpyH(G,args.node_num,args.edge_num),dtype=torch.float32)
            ...
            for sub_H,sub_full_idx,sub_eval_idx_ful,sub_eval_idx_sub,node2edge,edge2node in train_subgraphs:
                ...
                # if args.position == 1 and not args.snn == args.node_num:
                #     node2edge,edge2node = node2edge.to(device),edge2node.to(device)
                optimizer.zero_grad()
                Z = model(device,_sub_X_,sub_H,sub_full_idx,node2edge,edge2node)
                loss = F.nll_loss(Z[sub_eval_idx_sub], Y[sub_eval_idx_ful])
