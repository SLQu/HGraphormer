...
if __name__ == '__main__':
    ...
    log.info(f"Run {run}/{args.nruns},best test accuracy: {best_test_acc:.4f}, acc(last): {test_acc:.4f}, total time: {time.time()-tic_run:.2f}s")
    test_accs.append(test_acc)
    best_test_accs.append(best_test_acc)
    
    # log.info(f"Average final test accuracy: {np.mean(test_accs)} ± {np.std(test_accs)}")
    log.info(args.cmd_input)
    log.info(f"Average best test accuracy: {np.mean(best_test_accs)} ± {np.std(best_test_accs)}")
    

    # _acc_mean = str(np.mean(best_test_accs))
    # _acc_std = ','+str(np.std(best_test_accs))
